...
default_config = "M:/lss/tools/OpenColorIO-Configs-master/aces_1.2/config.ocio"


def proj_name_from_path(media):
    if media is not None:
        return re.split(os.path.sep + "\\|/", media)[1]
    else:
        return None
...
def ocio_config_from_media(media, attributes):
    ...
    if mapping_path is None:
        # print("Mapping not found, selecting default config.")
        return config.CreateFromFile(default_config)

      
def is_raw(media):
    # return media is not None and \
    #     os.path.splitext(media)[1] in [".avi", ".png", ".jpg", ".jpeg", ".mov"]
    filename = os.path.basename(media)
    isTure = filename.find("Utility - Raw") != -1
    return media is not None and isTure
    # return media is not None and  os.path.splitext(media)[1] in [".avi", ".jpg", ".jpeg", ".mov",".png"]

def is_sRGB(media):
    filename = os.path.basename(media)
    isTure = filename.find("Utility - sRGB") != -1
    return media is not None and isTure
    # return media is not None and  os.path.splitext(media)[1] in [".avi", ".jpg", ".jpeg", ".mov",".png"]
...
def ocio_node_from_media(config, node, default, media=None, attributes={}):
    ...
    global is_media_raw
    global is_media_srgb
    # print("nodeType:%s" %nodeType)
    if media is not None:
        is_media_raw = is_raw(media)
        is_media_srgb = is_sRGB(media)

    if is_media_raw:
        working_space = config.getColorSpace(OCIO.Constants.ROLE_DATA).getName()
    # elif is_media_srgb:
    #     working_space = config.getColorSpace(OCIO.Constants.ROLE_SCENE_LINEAR).getName()
    else:
        working_space = config.getColorSpace(OCIO.Constants.ROLE_SCENE_LINEAR).getName()
    
    if is_media_raw:
        sourceColorSpace = config.getColorSpace(OCIO.Constants.ROLE_DATA).getName()
    elif is_media_srgb:
        sourceColorSpace = config.getColorSpace(OCIO.Constants.ROLE_MATTE_PAINT).getName()
    else:
        sourceColorSpace = config.getColorSpace(OCIO.Constants.ROLE_SCENE_LINEAR).getName()
